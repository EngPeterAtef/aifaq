...
def main():
    ...
    st.set_page_config(page_title="AI FAQ", page_icon="🤖", layout="wide")
    # load model
    ...
    documents = filter_complex_metadata(
        documents, allowed_types=(str, int, float, bool)
    )

    # split the documents in small chunks
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=1000, chunk_overlap=100
    )  # Chage the chunk_size and chunk_overlap as needed
    ...
    # streamlit app
    st.title("Zephyr-7b-alpha-sharded")
    st.write("A conversational model based on zephyr-7b-alpha-sharded")
    # create text input
    ...
    if query or submit_btn:
        msg, chatbot = create_conversation(query, [])
        st.write(msg)
        st.write(chatbot)
